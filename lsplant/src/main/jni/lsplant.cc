...
namespace lsplant {
    ...
    // Library paths for trampoline memory disguise
    constexpr const char *kLibraryPath64 = "/system/lib64/libz.so";
    constexpr const char *kLibraryPath32 = "/system/lib/libz.so";
    std::vector<uintptr_t> trampolineUpdateCache;
    ...
    namespace {
        ...
        consteval inline auto GetTrampoline() {
            ...
            if constexpr (kArch == Arch::kX86_64) {
                return std::make_tuple(
                        "\x48\xbf\x78\x56\x34\x12\x78\x56\x34\x12\xff\x77\x00\xc3"_uarr,
                        // NOLINTNEXTLINE
...
// for old platform
        ...
        constexpr auto kInternalMethods = std::make_tuple(
                &method_get_name, &method_get_declaring_class, &class_get_name,
                &class_get_class_loader,
                &class_get_declared_constructors, &dex_file_init, &dex_file_init_with_cl,
                &load_class,
        ...
        bool InitJNI(JNIEnv *env) {
            ...
            if (sdk_int >= __ANDROID_API_O__) {
                executable = JNI_NewGlobalRef(env,
                                              JNI_FindClass(env, "java/lang/reflect/Executable"));
            } else {
                executable = JNI_NewGlobalRef(env, JNI_FindClass(env,
            ...
            if (method_get_declaring_class =
                        JNI_GetMethodID(env, executable, "getDeclaringClass",
                                        "()Ljava/lang/Class;");
            ...
            if (method_get_parameter_types =
                        JNI_GetMethodID(env, executable, "getParameterTypes",
                                        "()[Ljava/lang/Class;");
            ...
            if (method_get_return_type =
                        JNI_GetMethodID(env, JNI_FindClass(env, "java/lang/reflect/Method"),
                                        "getReturnType",
                                        "()Ljava/lang/Class;");
            ...
            if (class_get_declared_constructors = JNI_GetMethodID(env, clazz,
            ...
            if (class_access_flags = JNI_GetFieldID(env, clazz, "accessFlags",
                                                    "I"); !class_access_flags) {
            ...
            if (sdk_int >= __ANDROID_API_Q__) {
                ...
                dex_file_init = JNI_GetMethodID(env, dex_file_class, "<init>",
        ...
        BuildDex(JNIEnv *env, jobject class_loader,
            ...
            auto hook_builder{cbuilder.CreateMethod(
                    generated_method_name == "{target}"sv ? method_name.data()
            ...
            hook_builder.AddInstruction(
                    Instruction::InvokeVirtualObject(handle_hook_method.id, tmp, tmp,
                                                     hook_params_array));
            ...
            if (auto dex_file_class = JNI_FindClass(env,
                                                    "dalvik/system/DexFile"); dex_file_init_with_cl) {
                java_dex_file = JNI_NewObject(
                        env, dex_file_class, dex_file_init_with_cl,
                        JNI_NewObjectArray(
                                env, 1, JNI_FindClass(env, "java/nio/ByteBuffer"),
                                JNI_NewDirectByteBuffer(env, const_cast<void *>(image.ptr()),
                                                        image.size())),
                        nullptr, nullptr);
                ...
                void *target =
                        mmap(nullptr, image.size(), PROT_WRITE | PROT_READ,
                ...
                const auto *dex = DexFile::OpenMemory(
                        ...
                        generated_source_name.empty() ? "lsplant" : generated_source_name,
                        &err_msg);
                if (!dex) {
        ...
        void *GenerateTrampolineFor(art::ArtMethod *hook) {
            ...
            uintptr_t address;
            while (true) {
                auto tl = Trampoline{.address = trampoline_pool.fetch_add(1,
                ...
                if (address == 0 || count >= kTrampolineNumPerPage) {
                    ...
                    // Open a system library to disguise the memory mapping in /proc/self/maps
                    int disguise_fd = open(kLibraryPath64, O_RDONLY);
                    if (disguise_fd == -1) {
                        disguise_fd = open(kLibraryPath32, O_RDONLY);
                    }
                    LOGV("Disguise library fd = %d", disguise_fd);


                    address = reinterpret_cast<uintptr_t>(mmap(nullptr, kPageSize,
                                                               PROT_READ | PROT_WRITE | PROT_EXEC,
                                                               MAP_ANONYMOUS | MAP_PRIVATE, -1, 0));
                    trampolineUpdateCache.push_back(address);
            ...
            *reinterpret_cast<art::ArtMethod **>(address_ptr + art_method_offset) = hook;
            ...
            __builtin___clear_cache(address_ptr,
                                    reinterpret_cast<char *>(address + trampoline.size()));
        ...
        bool DoHook(ArtMethod *target, ArtMethod *hook, ArtMethod *backup) {
            ...
            LOGV("Hooking: target = %s(%p), hook = %s(%p), backup = %s(%p)",
                 target->PrettyMethod().c_str(),
                 target, hook->PrettyMethod().c_str(), hook, backup->PrettyMethod().c_str(),
                 backup);
            ...
            if (auto *entrypoint = GenerateTrampolineFor(hook); !entrypoint) {
                ...
                LOGV("Done hook: target(%p:0x%x) -> %p; backup(%p:0x%x) -> %p; hook(%p:0x%x) -> %p",
                     target,
                     target->GetAccessFlags(), target->GetEntryPoint(), backup,
                     backup->GetAccessFlags(),
        ...
        std::string GetProxyMethodShorty(JNIEnv *env, jobject proxy_method) {
            const auto return_type = JNI_CallObjectMethod(env, proxy_method,
                                                          method_get_return_type);
            const auto parameter_types =
                    JNI_Cast<jobjectArray>(
                            JNI_CallObjectMethod(env, proxy_method, method_get_parameter_types));
    ...
    inline namespace v2 {
        ...
        [[maybe_unused]] void updateTrampolineToOnlyRead() {
            for (size_t i = 0; i < trampolineUpdateCache.size(); ++i) {
                mprotect((void *) trampolineUpdateCache[i], 0x1000, PROT_READ | PROT_EXEC);
            }
        }
        ...
        [[maybe_unused]] jobject Hook(JNIEnv *env, jobject target_method, jobject hooker_object,
            ...
            bool is_proxy =
                    JNI_GetIntField(env, target_class, class_access_flags) & kAccClassIsProxy;
            ...
            ScopedLocalRef <jclass> built_class{env};
            {
                ...
                auto target_name =
                        JNI_Cast<jstring>(
                                JNI_CallObjectMethod(env, target_method, method_get_name));
                ...
                auto callback_class_name =
                        JNI_Cast<jstring>(
                                JNI_CallObjectMethod(env, callback_class, class_get_name));
                ...
                std::tie(built_class, hooker_field, hook_method, backup_method) = WrapScope(
                        ...
                        BuildDex(env, callback_class_loader.get(),
                                 __builtin_expect(is_proxy, 0) ? GetProxyMethodShorty(env,
                                                                                      target_method)
                                                               : ArtMethod::GetMethodShorty(env,
                                                                                            target_method),
                                 is_static,
            ...
            auto reflected_backup = JNI_ToReflectedMethod(env, built_class, backup_method,
                                                          is_static);
            ...
            if (DoHook(target, hook, backup)) {
                std::apply(
                        [backup_method, target_method_id = env->FromReflectedMethod(target_method)](
                                auto... v) {
                            ((*v == target_method_id &&
                              (LOGD(
                                "Propagate internal used method because of hook"), *v = backup_method)) ||
                            ...);
                        },
                        kInternalMethods);
                jobject global_backup = JNI_NewGlobalRef(env, reflected_backup);
                RecordHooked(target, target->GetDeclaringClass()->GetClassDef(), global_backup,
                             backup);
                if (!is_proxy) [[likely]] {
        ...
        [[maybe_unused]] bool UnHook(JNIEnv *env, jobject target_method) {
            ...
            hooked_classes_.erase_if(target->GetDeclaringClass()->GetClassDef(),
                                     [&target](auto &it) {
            ...
            if (DoUnHook(target, backup)) {
                std::apply(
                        [backup_method, target_method_id = env->FromReflectedMethod(target_method)](
                                auto... v) {
                            ((*v == backup_method && (LOGD(
                                "Propagate internal used method because of unhook"),
                            *v = target_method_id)) ||
                            ...);
                        },
                        kInternalMethods);
        ...
        [[maybe_unused]] bool MakeClassInheritable(JNIEnv *env, jclass target) {
            ...
            const auto constructors =
                    JNI_Cast<jobjectArray>(
                            JNI_CallObjectMethod(env, target, class_get_declared_constructors));
            uint8_t access_flags = JNI_GetIntField(env, target, class_access_flags);
            constexpr static uint32_t kAccFinal = 0x0010;
            JNI_SetIntField(env, target, class_access_flags,
                            static_cast<jint>(access_flags & ~kAccFinal));
